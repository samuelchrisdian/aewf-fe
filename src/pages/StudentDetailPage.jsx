...
import { AlertOctagon, CheckCircle, ArrowLeft, Calendar, BookOpen } from 'lucide-react';
...
const StudentDetailPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await getStudentDetail(nis);
        ...
        datasets: [
            {
                ...
                data: student.weeklyTrend.map(val => val * 100),
                borderColor: '#1a56db',
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <div className={`px-4 py-2 rounded-lg border flex flex-col items-center ${student.riskLevel === 'HIGH' ? 'bg-red-50 border-red-100 text-red-700' :
                            student.riskLevel === 'MEDIUM' ? 'bg-yellow-50 border-yellow-100 text-yellow-700' :
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Left Column: Transparency & Stats */}
                <div className="lg:col-span-2 space-y-6">
                    {/* Triggered Rules */}
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center">
                            <AlertOctagon className="w-5 h-5 mr-2 text-primary" />
                            Risk Factors (Explainability)
                        </h3>
                        <div className="space-y-3">
                            {student.triggeredRules.map((rule, idx) => (
                                <div key={idx} className="flex items-start p-3 bg-red-50 border border-red-100 rounded-lg">
                                    <div className="min-w-[4px] h-4 mt-1 bg-red-500 rounded-full mr-3"></div>
                                    <p className="text-sm text-gray-800 font-medium">{rule}</p>
                                </div>
                            ))}
                            {student.triggeredRules.length === 0 && (
                                <p className="text-gray-500 italic">No specific risk rules triggered currently.</p>
                            )}
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-full">
                        ...
                        <ul className="space-y-4">
                            {student.recommendations.map((rec, idx) => (
                                <li key={idx} className="flex items-start text-sm text-gray-700">
                                    <span className="flex items-center justify-center w-5 h-5 rounded-full bg-blue-100 text-blue-600 text-xs font-bold mr-3 mt-0.5">
                                        {idx + 1}
                                    </span>
                                    {rec}
                                </li>
                        ...
                        <button className="mt-6 w-full py-2 bg-primary text-white rounded-lg font-medium hover:bg-blue-700 transition-colors">
                            Mark as Contacted
                        </button>
                    </div>
                </div>
